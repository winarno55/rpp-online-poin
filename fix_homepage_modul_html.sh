#!/bin/bash
# Remove markdownToHtml wrapping
sed -i 's/setModulHtml(markdownToHtml(accumulatedMarkdown));/setModulHtml(accumulatedMarkdown);/g' src/pages/HomePage.tsx

# Add Doc Download function inside HomePage.tsx Modul Ajar section

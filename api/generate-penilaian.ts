...
import { generatePenilaianPrompt } from '../src/services/geminiService.js';
import { PenilaianInput } from '../src/types.js';
import { COST_ASISTEN_PENILAIAN } from '../src/constants.js';
...
async function apiHandler(req: AuthRequest, res: VercelResponse) {
    try {
        ...
        const penilaianData: PenilaianInput = req.body;
        
        let feedback;
        try {
            const prompt = generatePenilaianPrompt(penilaianData);
            ...
            if (aiError.message && aiError.message.toLowerCase().includes('safety')) {
                userMessage = 'Permintaan Anda diblokir oleh filter keamanan AI. Coba ubah teks atau kriteria Anda.';
            }
